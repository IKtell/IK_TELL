...
import com.robot.tuling.ui.ChooseRoleActivity;
...
public class Camera2BasicFragment extends Fragment
     ...
     */
    private static final int MAX_PREVIEW_HEIGHT = 1980;
        ...
        private void process(CaptureResult result) {
            switch (mState) {
                ...
                case STATE_WAITING_LOCK: {
                    @SuppressLint({"NewApi", "LocalSuppress"}) Integer afState = result.get(CaptureResult.CONTROL_AF_STATE);
                    if (afState == null) {
                        captureStillPicture();
                    } else if (CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED == afState ||
                            CaptureResult.CONTROL_AF_STATE_NOT_FOCUSED_LOCKED == afState) {
                        // CONTROL_AE_STATE can be null on some devices
                        @SuppressLint({"NewApi", "LocalSuppress"}) Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                        if (aeState == null ||
    ...
    private static Size chooseOptimalSize(Size[] choices, int textureViewWidth,
        ...
        @SuppressLint({"NewApi", "LocalSuppress"}) int w = aspectRatio.getWidth();
        @SuppressLint({"NewApi", "LocalSuppress"}) int h = aspectRatio.getHeight();
    ...
    private void setUpCameraOutputs(int width, int height) {
        Activity activity = getActivity();
        @SuppressLint({"NewApi", "LocalSuppress"}) CameraManager manager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        try {
    ...
    private void captureStillPicture() {
        try {
            ...
            @SuppressLint({"NewApi", "LocalSuppress"}) CameraCaptureSession.CaptureCallback CaptureCallback
                    = new CameraCaptureSession.CaptureCallback() {
